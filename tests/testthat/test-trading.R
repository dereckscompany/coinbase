...
test_that("parse_orders flattens config into columns with no list columns", {
  items <- list(
    list(order_id = "o1", client_order_id = "c1", product_id = "BTC-USD",
         side = "BUY", status = "FILLED", time_in_force = "GTC",
         created_time = "2026-01-01T00:00:00Z", completion_percentage = "100",
         filled_size = "0.001", average_filled_price = "50000", number_of_fills = "1",
         filled_value = "50", total_fees = "0.3",
         order_configuration = list(limit_limit_gtc = list(base_size = "0.001", limit_price = "50000"))),
    list(order_id = "o2", product_id = "ETH-USD", side = "SELL", status = "OPEN",
         order_configuration = list(market_market_ioc = list(quote_size = "25")))
  )
  dt <- parse_orders(items)
  expect_equal(nrow(dt), 2L)
  expect_true(all(c("order_id","product_id","side","status","config_type","limit_price","base_size","quote_size") %in% names(dt)))
...
test_that("parse_fills coerces types with no list columns", {
  items <- list(
    list(entry_id = "e1", trade_id = "t1", order_id = "o1", product_id = "BTC-USD",
         side = "BUY", trade_time = "2026-01-01T00:00:00Z", trade_type = "FILL",
         price = "50000", size = "0.001", commission = "0.3", liquidity_indicator = "MAKER")
  )
...
test_that("parse_preview flattens result and joins error list to a string", {
  data <- list(order_total = "10", commission_total = "0.118", quote_size = "9.88",
               base_size = "0.00013", best_bid = "73900", best_ask = "73901",
               slippage = "0.0001", errs = list("PREVIEW_INSUFFICIENT_FUND"),
               preview_id = "p1")
