...
test_that("parse_accounts flattens {value,currency} and has no list columns", {
  items <- list(
    list(
      ...
      active = TRUE, default = TRUE, ready = TRUE,
      type = "ACCOUNT_TYPE_CRYPTO", platform = "ACCOUNT_PLATFORM_CONSUMER",
      retail_portfolio_id = "p1",
      created_at = "2025-01-01T00:00:00Z", updated_at = "2025-02-01T00:00:00Z"
    ),
    list(
      uuid = "u2", name = "USD Wallet", currency = "USD",
      available_balance = list(value = "100", currency = "USD"),
      hold = list(value = "0", currency = "USD"),
      active = TRUE, default = FALSE, ready = TRUE,
      type = "ACCOUNT_TYPE_FIAT", platform = "ACCOUNT_PLATFORM_CONSUMER",
      retail_portfolio_id = "p1",
      created_at = "2025-01-01T00:00:00Z", updated_at = "2025-02-01T00:00:00Z"
    )
  )
...
test_that("parse_fees flattens the fee_tier into scalar columns", {
  data <- list(
    total_volume = 1234.5, total_fees = 6.7, total_balance = "89.0",
    fee_tier = list(
      pricing_tier = "Intro 1",
      maker_fee_rate = "0.006", taker_fee_rate = "0.012",
      usd_from = "0", usd_to = "1000"
    )
  )
  dt <- parse_fees(data)
...
test_that("coinbase_paginate_cursor walks pages until has_next is false", {
  ...
  res <- coinbase_paginate_cursor(
    endpoint = "/x", items_field = "accounts",
    .req_fn = req_fn, .parser = function(items) length(items)
  )
  expect_equal(res, 5L)
...
test_that("coinbase_paginate_cursor respects max_pages", {
  ...
  req_fn <- function(endpoint, query) page
  res <- coinbase_paginate_cursor(
    endpoint = "/x", items_field = "accounts", max_pages = 2,
    .req_fn = req_fn, .parser = function(items) length(items)
  )
