...
CoinbaseAccount <- R6::R6Class(
  ...
  public = list(
    ...
    #'   thereof.
    #'   - uuid (character) the portfolio UUID.
    #'   - name (character) the portfolio's display name.
    #'   - type (character) the portfolio type, e.g. `"DEFAULT"`.
    #'   - deleted (logical) whether the portfolio has been deleted.
    ...
    #'   thereof.
    #'   - can_view (logical) whether the key may read account data.
    #'   - can_trade (logical) whether the key may place and cancel orders.
    #'   - can_transfer (logical) whether the key may move funds.
    #'   - portfolio_uuid (character) the key's portfolio UUID.
    #'   - portfolio_type (character) the portfolio type, e.g. `"DEFAULT"`.
