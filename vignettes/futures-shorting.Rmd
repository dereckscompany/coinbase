...
> contract expiry add further risk. Every example below uses `eval = FALSE` and
> a **dry-run preview** for any order — nothing here ever transmits a live
> order. Trade futures only with risk capital you can afford to lose, and never
> run a live order example without understanding it fully.
...
# COINBASE_API_KEY_NAME="organizations/00000000-0000-0000-0000-000000000000/apiKeys/11111111-1111-1111-1111-111111111111"
...
```{r position}
pos <- futures$get_position(product_id = "BIT-28FEB25-CDE")
...
Futures products appear in the same product catalogue as spot. List products
and pick out the expiring CFM contracts (their IDs carry an expiry, e.g.
`BIT-28FEB25-CDE`):

```{r products}
products <- market$get_products()
products[grepl("-CDE$", product_id), .(product_id, status)]
```

Inspect a single contract's metadata before trading it:

```{r product}
contract <- market$get_product(product_id = "BIT-28FEB25-CDE")
print(contract)
```
...
```{r edit-cancel, eval = FALSE}
# Move the limit short to a new price
trading$edit_order(order_id = "your-order-id", price = 96000)

# Cancel one or more orders
trading$cancel_orders(order_ids = "your-order-id")
```
