...
# httr2 exposes a global mock hook that intercepts every req_perform /
# req_perform_promise call. connectcore::local_mock_api installs the shared
# router built from the coinbase route table, so every chunk below executes
# against canned, deterministic fixtures -- no network, no real credentials, no
# funds.
box::use(
...
table from `get_portfolio_summary()` (same endpoint) — no attributes,
no nested lists:
...
`data.table` with `timestamp` (`POSIXct`), `price` (`numeric`), and `size`
...
- **No reconnect / retry on transient network errors.** The single
  call is what you asked for. The low-level `coinbase_build_request()`
  takes a `.perform` argument, so you can supply one that wraps the
  request in `httr2::req_retry()` before performing it.
