...
Then in R, using `box::use` rather than `library()`:
...
are snake_case. Typical columns include `id`, `base_currency`,
`quote_currency`, `display_name`, `status`, `quote_increment`,
`base_increment`, `min_market_funds`, `trading_disabled`,
`fx_stablecoin`, `auction_mode`. Numeric-looking fields are returned
...
# Level 3 carries order_id instead of num_orders
...
`trade_id`) left as the API sends them. `get_server_time()` returns a
single row with `iso` and `epoch`.
...
`as_dt_list` flattener (columns such as `name`, `uuid`, `type`,
`deleted`). `get_key_permissions()` returns a single row via
`as_dt_row` (columns such as `can_view`, `can_trade`,
`can_transfer`, `portfolio_uuid`, `portfolio_type`).
...
orders[status == "FILLED" & average_filled_price > 50000,
       .(order_id, side, filled_size, average_filled_price)]
```
...
pv[, .(order_total, commission_total, base_size, errs)]
```
...
(e.g. `"BIT-28FEB25-CDE"`) and a futures order configuration; this
...
```{r}
box::use(coinbase[get_api_keys, get_base_url, get_exchange_base_url])
...
```{r}
box::use(coinbase[verify_symbol, generate_client_order_id])
...
#> 1: bid  42950.50  0.5      3
#> 2: bid  42949.00  1.2      5
#> 3: ask  42951.00  0.3      2
#> 4: ask  42952.50  0.8      1
```
...
pv <- trading$preview_order(
  "BTC-USD", "BUY",
  list(limit_limit_gtc = list(base_size = "0.001", limit_price = "1"))
)
pv$errs
#> "PREVIEW_INSUFFICIENT_FUND"     # one string, never a list cell
