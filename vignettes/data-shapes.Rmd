...
and increments (`base_increment` / `quote_increment` / `price_increment`).
The parser (`parse_products`) selects a curated, typed set of columns (one
row per product); `get_product()` is the single-row form of the same
shape. Numeric-looking fields are returned as the character strings
Coinbase sends — cast at the point of use.
