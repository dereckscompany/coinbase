...
`coro::async()` / `await()` for sequential-looking code, and drive the
event loop with [later](https://r-lib.github.io/later/).
...
`e$body_snippet`). Catch a specific class with `tryCatch()`, or a whole
...
1.  `vignette("getting-started", package = "coinbase")` — constructing
    clients, reading public market data, checking authenticated account
    state, and safely previewing orders against the Coinbase Advanced
    Trade API, in synchronous mode.
2.  `vignette("async-usage", package = "coinbase")` — the same calls in
    asynchronous mode, consuming `promises` with `coro::async()` /
    `await()` and driving the event loop with `later::run_now()`.
3.  `vignette("data-shapes", package = "coinbase")` — a method-by-class
    catalogue of the exact `data.table` shape every call returns, and
    the data-shape conventions shared with the sister `alpaca`,
    `binance`, and `kucoin` packages.
4.  `vignette("futures-shorting", package = "coinbase")` — going short
    through Coinbase Financial Markets (CFM) US futures, since Coinbase
    spot is long-only.
