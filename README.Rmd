...
**Coinbase is a large, US-based cryptocurrency exchange where people buy, sell and hold digital coins.** This package is the R doorway to Coinbase's Advanced Trade service: you can pull market prices and history, look at your account and balances, and place, preview and manage buy and sell orders on both the spot market and Coinbase's US futures -- all from R scripts. Every request can run the ordinary way, where your code waits for the answer, or in the background so your program keeps working and collects the result later. It also bundles small conveniences such as turning a raw stream of individual trades into evenly spaced price candles and downloading long stretches of history in bulk. Because it can place real orders, it offers preview methods that check an order without sending it, so you can test your logic safely first.
...
- **Sync and async.** Every method works in both modes. `async = TRUE` returns a [promise](https://rstudio.github.io/promises/); otherwise results are returned directly. There is a single sync/async branch point.
...
[promise](https://rstudio.github.io/promises/) instead of a `data.table`. Resolve it with
...
A bad argument is caught before any request as a `coinbase_validation_error` -- the connector's own domain root, raised for a malformed `product_id`, an edit with neither `price` nor `size`, or missing credentials, before any network call happens. An HTTP failure returned by Coinbase itself surfaces instead as a typed `coinbase_api_error_<status>`, rolling up to `coinbase_api_error` and then connectcore's fleet-wide `connectcore_api_error_<status>` / `connectcore_api_error` / `connectcore_error` classes, carrying the HTTP status, the request URL (credentials redacted), and a snippet of the response body as structured fields (`e$status`, `e$url`, `e$body_snippet`). Catch a specific class with `tryCatch()`, or a whole family by its root, instead of matching on message text.
...
The rendered reference site is at [dereckscompany.github.io/coinbase](https://dereckscompany.github.io/coinbase).

Four vignettes take a reader from a first call to shorting through futures, in this reading order:

1. `vignette("getting-started", package = "coinbase")` — constructing clients, reading public market data, checking authenticated account state, and safely previewing orders against the Coinbase Advanced Trade API, in synchronous mode.
2. `vignette("async-usage", package = "coinbase")` — the same calls in asynchronous mode, consuming `promises` with `coro::async()` / `await()` and driving the event loop with `later::run_now()`.
3. `vignette("data-shapes", package = "coinbase")` — a method-by-class catalogue of the exact `data.table` shape every call returns, and the data-shape conventions shared with the sister `alpaca`, `binance`, and `kucoin` packages.
4. `vignette("futures-shorting", package = "coinbase")` — going short through Coinbase Financial Markets (CFM) US futures, since Coinbase spot is long-only.
...
> Mezquita, D. (2026). coinbase: API Wrapper to Coinbase Advanced Trade. R package version 0.8.4. https://github.com/dereckscompany/coinbase.
